...
const Place: React.FC<PlaceProp> = ({
    ...
    return (
        <li className='md:h-260px flex flex-col md:flex-row gap-3 py-3 border-b border-solid border-grey'>
            <Link to={`/places/${_id?.toString()}`} className='md:w-1/2 h-full rounded-lg overflow-hidden shadow-image'>
                ...
                <img className='h-200px sm:h-240px md:h-full w-full object-contain cursor-pointer transition-transform duration-300 hover:scale-110' src={images[1]} alt="place" />
                <img className='h-200px sm:h-240px md:h-full w-full object-contain cursor-pointer transition-transform duration-300 hover:scale-110' src={images[2]} alt="place" />
            </Link>
            <div className='md:w-1/2'>
                ...
                <p className='flex items-center gap-2 text-18px md:text-19px font-medium text-normal'>
                    <i className="ri-time-line text-22px text-secondarydark"></i> {type}
                </p>
